...
public class Staffs implements Item, Serializable, Cloneable, Exportable, LibraryItem, Importable, TreasureItem {
	...
	public String existsAs(){
		...
		Vector<Staffs> v = db.selectStaffs(this);
		if(v.size() > 0){
			return ((Staffs)v.get(0)).getName();
		}
		return null;
	}
	
	public Vector<Staffs> getValues(){
		StaffsDAO db = new StaffsDAO();
		return db.getStaffs();
	}
