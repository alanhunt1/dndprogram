...
public class TestTabUI extends TabbedPaneUI implements SwingConstants {

	// Instance variables initialized at installation
	private ImageIcon bgImage = new ImageIcon("images/wood.jpg");

	private ImageIcon tabImage = InitImage.tabGraphic;
	...
	protected void installKeyboardActions() {
		...
		LazyActionMap.installLazyActionMap(tabPane, TestTabUI.class,
	 ...
	 * <code>index</code> will be -1 if the mouse is no longer over any tab.
	 * No checking is done to ensure the passed in index identifies a valid tab.
	 * 
	 * @param index
	 *            Index of the tab the mouse is over.
	 * @since 1.5
	...
	protected void paintTab(Graphics g, int tabPlacement, Rectangle[] rects,
		...
		g.drawImage(tabImage.getImage(), selRect.x, selRect.y, selRect.width,
				selRect.height, null);
			...
			if (bgImage != null){
				...
				for (int i = 0; i < xTiles+1; i++){
						for (int j = 0; j < yTiles+1; j++){
								//Draw image 
								g.drawImage(bgImage.getImage(), x+(i*imgW), y+(j*imgH), null);
						}
				}
		...
		case LEFT: {
			if (run == 0) {
				...
				offset = (forward ? maxTabWidth : calculateTabAreaWidth(
						tabPlacement, runCount, maxTabWidth)
						- maxTabWidth);
			} else {
	...
	boolean requestFocusForVisibleComponent() {
		...
		if (visibleComponent != null && visibleComponent.isFocusable()) {
			...
			FocusTraversalPolicy policy = ((Container)visibleComponent).getFocusTraversalPolicy();
			if (policy.getDefaultComponent((Container)visibleComponent).requestFocusInWindow()){
				return true;
			}
	...
	private static class Actions extends UIAction {
		...
		public void actionPerformed(ActionEvent e) {
			...
			TestTabUI ui = (TestTabUI) getUIOfType(pane.getUI(),
					TestTabUI.class);
	...
	private class ScrollableTabPanel extends JPanel implements UIResource {
		...
		public void paintComponent(Graphics g) {
			...
			TestTabUI.this.paintTabArea(g, tabPane.getTabPlacement(), tabPane
					.getSelectedIndex());
			...
			if (scrollableTabLayoutEnabled()) {
				...
				if (index < rects.length && index != -1) {
					tabScroller.tabPanel
							.scrollRectToVisible((Rectangle) rects[index]
									.clone());
				}
			}
		}
	...
	/**
	 * Returns the ui that is of type <code>klass</code>, or null if one can
	 * not be found.
	 */
