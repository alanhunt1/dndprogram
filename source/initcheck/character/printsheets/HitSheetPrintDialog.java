...
public class HitSheetPrintDialog extends JFrame {
	...
	Vector<HitSheetPrinter> hitSheets = new Vector<HitSheetPrinter>();
	...
	HitSheetPrinter currentHitSheet;

	JLabel pageLabel;

	public HitSheetPrintDialog(PlayerManager owner, String color,
		...
		for (int i = 0; i < chars.size(); i++) {
			...
			if (i > 0 && i % (maxChars - 1) == 0) {
				...
				HitSheetPrinter hsp = new HitSheetPrinter(tmpVector);
				if (c != null) {
					hsp.setColor(c);
				}
				hitSheets.add(hsp);
		...
		pageLabel = new JLabel("Page " + page + " of " + hitSheets.size());
		...
		printScroll = new JScrollPane((HitSheetPrinter) hitSheets.get(0));
		currentHitSheet = (HitSheetPrinter) hitSheets.get(0);
	...
	private void goPrevious() {
		if (page > 1) {
			page--;
			pageLabel.setText("Page " + page + " of " + hitSheets.size());
			printScroll.setViewportView((HitSheetPrinter) hitSheets
					.get(page - 1));
	...
	private void printSheet() {
		((HitSheetPrinter) hitSheets.get(page - 1)).printSheet();
	}
	...
	public void printAllSheets() {
		...
		pf.setPaper(paper);
		for (int i = 0; i < hitSheets.size(); i++) {
			book.append((HitSheetPrinter) hitSheets.get(i), pf);
		}
